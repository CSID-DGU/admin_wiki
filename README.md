...
이 디렉터리는 Markdown 원본, PDF 산출물, Docker 배포 구성을 한곳에서 관리한다.
...
각 관리자는 자기 계정의 clone에서 `wiki/md/`를 수정해 push한다. 공용 컨테이너는
...
```bash
python3 wiki/manage.py up
python3 wiki/manage.py status
python3 wiki/manage.py logs sync --follow
python3 wiki/manage.py sync-now
python3 wiki/manage.py publish-local
python3 wiki/manage.py down
```
...
저장소가 public이면 별도 설정이 없다. private이면 `wiki/.env.example`을
`wiki/.env`로 복사하고 Contents read-only 권한의 fine-grained token을 넣는다.
`.env`는 Git에서 제외된다.

```bash
cp wiki/.env.example wiki/.env
# wiki/.env의 WIKI_GITHUB_TOKEN 설정
python3 wiki/manage.py up
```
...
`wiki/md/` 아래의 Markdown만 직접 수정한다.

```bash
git pull --ff-only
# wiki/md/system/*.md 등 수정
git add wiki/md
git commit
git push
```
...
저장소의 `wiki/pdf/`에도 새 PDF를 남기고 싶을 때만 호스트 명령을 실행한다.
...
- `sync_wiki_docs.py`: `wiki/md/`를 MkDocs 입력 tree로 변환하고 PDF를 복사한다.
