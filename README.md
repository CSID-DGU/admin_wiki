...
이 저장소는 private이므로 `.env.example`을 `.env`로 복사하고 Contents
read-only 권한의 fine-grained token을 넣는다. `.env`는 Git에서 제외된다.
...
현재 컨테이너의 clone 요청에는 GitHub 자격증명이 필요하므로 위 read-only
token 설정이 필요하다. token은 wiki HTTP 인증과 무관하며 컨테이너가 문서
원본을 읽을 때만 사용한다.
