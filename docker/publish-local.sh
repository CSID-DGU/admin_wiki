...
source_dir=/source
work_dir=/tmp/server-manage-wiki-source
next_site=/tmp/server-manage-wiki-local
site_dir=/site

test -d "$source_dir/wiki/md"
rm -rf "$work_dir" "$next_site"
mkdir -p "$work_dir"
cp -R "$source_dir"/. "$work_dir"/

python3 "$work_dir/wiki/export_manuals.py"
python3 "$work_dir/wiki/sync_wiki_docs.py"
mkdocs build \
  --clean \
  --strict \
  --config-file "$work_dir/wiki/mkdocs.yml" \
  --site-dir "$next_site"
