...
이 문서의 역할은 코드를 한 줄도 보기 전에 "이 시스템이 왜 만들어졌고 Admin BE가 그 안에서 어떤 몫을 맡는지" 머릿속에 그림을 그려주는 것입니다. 세부 기능을 나열하기보다, 관리자의 반복 업무를 자동화한다는 목적 자체와 그 목적이 LAB·FARM이라는 이기종 서버 환경 때문에 왜 필요해졌는지를 먼저 설명합니다. 이 그림이 잡혀야 이후 문서들에서 다루는 개별 기능이 "무엇을 위해" 그렇게 설계됐는지 이해할 수 있습니다.
...
아직 로컬 개발 환경을 세팅하지 않았다면 [시작](시작.md)에서 먼저 진행합니다. IntelliJ 설정, `application.yml` 준비, 로컬 DB 설정, Swagger 확인까지 순서대로 안내합니다. 이미 세팅을 마쳤다면 다음 순서인 [시스템 아키텍처](시스템-아키텍처.md)로 넘어가 전체 흐름을 확인합니다.
