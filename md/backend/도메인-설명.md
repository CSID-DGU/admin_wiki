...
엔티티별로 어떤 필드를 어떤 형태로 저장하고 있는지, 그리고 그 데이터가 생성부터 삭제까지 어떤 생명주기를 거치는지 정리한 문서입니다. 필드 구성에 별도 근거가 있는 경우(예: Soft Delete 뒤에 즉시 지우지 않는 처리, 비밀번호를 두 가지 형태로 같이 저장하는 처리)에는 해당 절에서 이유를 함께 밝혔습니다. 그 배경을 모르고 필드를 손대면 다른 기능이 조용히 깨지므로, 새 필드를 추가하거나 마이그레이션을 준비하기 전에는 반드시 여기서 해당 엔티티 절을 먼저 확인합니다.
...
| `ubuntuPassword` | 초기 로그인 비밀번호. 생성 작업 등록 때 base64(`getUbuntuPasswordBase64()`)로 보내고 배정 안내 메일에 넣는다. **안내 메일을 보낸 뒤(발송 실패여도)와 거절할 때 빈 값으로 지운다**(컬럼이 NOT NULL). 이후 마이그레이션은 config-server가 기존 Pod의 비밀번호 Secret을 이어받는다 |
| `ubuntuUid` / `ubuntuGid` | 승인 후 Infra Server에서 받아 저장 |
| `podName` / `nodeName` | 승인 후 Pod 생성 결과 저장 |
| `expiresAt` | 컨테이너 만료일 |
| `status` | 현재 상태 (`PENDING` / `PROCESSING` / `FULFILLED` / `MIGRATING` / `EXPIRING` / `DENIED` / `DELETED`) |
| `adminComment` | 거절 사유 또는 관리자 메모 |
| `formAnswers` | 신청 양식 응답 (JSON). 양식 질문이 변경돼도 스키마 마이그레이션 없이 수용하기 위해 컬럼을 분리하지 않고 JSON으로 저장 |

상태 전이와 승인 흐름은 [시스템 아키텍처](시스템-아키텍처.md)를 참고합니다.
...
| 엔드포인트 | 역할 |
|-----------|------|
| `GET /api/groups` | 전체 그룹 목록 조회. 그룹 ID·그룹명 반환 |
| `POST /api/groups` | 새 그룹 생성 |

그룹 삭제·수정 API는 없습니다. 잘못 만든 그룹을 정리해야 한다면 MySQL에 직접 접속합니다([운영 가이드](운영-가이드.md) 7절).
...
1. 관리자가 신청 승인 → 생성 작업 등록(`POST /operations/provision`)
2. 제어기가 NodePort를 배정해 컨테이너를 만들고, 작업 결과의 `ports` 배열에 남긴다
3. 작업 결과 폴러가 배열을 순회하며 `PodExternalPort` row를 `request_id`에 묶어 저장(마이그레이션하면 새 포트로 교체)
...
Prometheus가 일시적으로 내려가도 API는 200을 반환합니다(빈 값 응답). 각 쿼리에 5초 타임아웃이 적용됩니다. Prometheus 연동 상세(WebClient 설정, 응답 구조 등)는 [외부 연동](외부-연동.md) 섹션 4를 참고합니다.
