...
삭제 스케줄의 동작은 이 섹션의 `UserSchedulerService` 설명을 기준으로 본다.
...
Redis List의 `RPUSH`(오른쪽 삽입)로 넣고 `LPOP`(왼쪽 꺼내기)으로 꺼내 FIFO 순서를 유지합니다. 관련 키와 저장 규칙은 [Redis 키 카탈로그](Redis-키-카탈로그.md)를 참고합니다.
