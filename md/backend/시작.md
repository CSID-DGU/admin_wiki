...
순서를 건너뛰면 나중에 어느 단계에서 막혔는지 찾기 어려우므로, 아래 순서를 그대로 따라갑니다. IntelliJ 설치 확인부터 `application.yml` 준비, 로컬 DB 연결, 인코딩 설정, Swagger 확인까지 다룹니다. 신규 관리자는 이 문서부터 진행한 뒤 [개요](개요.md)로 넘어갑니다.
