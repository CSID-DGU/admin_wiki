...
필터가 건너뛰는 화이트리스트 경로(`UNPROTECTED_PATHS`)는 [[운영 가이드|운영-가이드]] 화이트리스트 설정 섹션에서 관리합니다.
...
`Role`을 변경하는 별도 API는 제공하지 않습니다. 필요한 경우 MySQL에 직접 접속해 수정합니다. 접속 방법은 [[운영 가이드|운영-가이드]] MySQL 직접 접속 섹션을 참고합니다.
