...
새 코드를 작성하거나 리뷰할 때 따라야 할 규칙을 정리합니다. 패키지 구조, 클래스·메서드 네이밍, Entity·DTO 작성 규칙, CQS(Command Query Separation), 예외 처리, 로깅, 쿼리 최적화, PR·테스트 작성 방식까지 다룹니다. 코드에서 "왜 이렇게 짜여 있는지" 의아한 부분이 있다면 먼저 이 문서를 확인합니다.
