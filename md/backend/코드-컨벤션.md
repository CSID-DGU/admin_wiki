...
사람이 바뀌어도 코드는 한 사람이 짠 것처럼 보여야 유지보수가 가능합니다. 이 문서의 역할은 그 일관성을 지키기 위한 규칙과, 그 규칙이 왜 그렇게 정해졌는지의 근거를 함께 남기는 것입니다. 예를 들어 Entity에 `@Data`를 쓰지 않는 이유, Setter 대신 의도가 드러나는 메서드를 쓰는 이유처럼, 규칙만 외우기보다 이유를 알아야 애매한 새 상황에서도 같은 판단을 내릴 수 있습니다. 코드에서 "왜 이렇게 짜여 있지?" 싶은 부분이 있다면 먼저 이 문서에서 답을 찾습니다.
...
- 위치: [`DGU_AI_LAB.admin_be.global.common.BaseTimeEntity`](https://github.com/CSID-DGU/admin_be/blob/develop/src/main/java/DGU_AI_LAB/admin_be/global/common/BaseTimeEntity.java)
