...
패키지 구조, 네이밍, 계층별 작성 규칙, 커밋·PR 절차까지 이 저장소가 실제로 지키고 있는 규칙을 정리했습니다. Entity에 `@Data`를 쓰지 않는다거나 Setter 대신 의도가 드러나는 메서드를 쓴다는 규칙처럼 근거가 필요한 항목은 규칙을 먼저 제시하고 이유를 이어서 덧붙였습니다. 규칙만 외우면 애매한 새 상황에서 판단이 서지 않으므로 이유까지 같이 알아둡니다. 코드에서 "왜 이렇게 짜여 있지?" 싶은 부분이 있다면 먼저 여기서 답을 찾습니다.
...
- 위치: [`DGU_AI_LAB.admin_be.global.common.BaseTimeEntity`](https://github.com/CSID-DGU/admin_be/blob/main/src/main/java/DGU_AI_LAB/admin_be/global/common/BaseTimeEntity.java)
...
위치: [`DGU_AI_LAB.admin_be.error.ErrorCode`](https://github.com/CSID-DGU/admin_be/blob/main/src/main/java/DGU_AI_LAB/admin_be/error/ErrorCode.java)

새 에러 케이스가 생기면 enum에 추가합니다. 유사한 상태 코드끼리 묶어서 작성하고, 메시지는 FE에 바로 노출될 수 있으므로 명확한 한글로 작성합니다. 현재 정의된 59개 케이스 전체 목록은 [에러 코드 카탈로그](에러-코드-카탈로그.md)를 참고합니다.
...
기능 개발, 버그 수정 가릴 것 없이 반드시 PR을 남기고 팀원에게 코드 리뷰를 받은 뒤 머지합니다. 리뷰 없이 main에 직접 푸시하지 않습니다. 브랜치·배포 규칙은 [공통 문서](../브랜치-규칙.md)를 따릅니다.
...
CI(`deploy.yml`)는 배포 속도를 위해 `./gradlew build -x test`로 테스트를 건너뜁니다. 즉 테스트 통과 여부는 CI가 아니라 PR 리뷰와 로컬 실행에 전적으로 의존합니다. 커밋 전 테스트 실행이 규칙(14절)인 이유이기도 합니다.
