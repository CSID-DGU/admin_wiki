...
이 문서의 목적은 같은 문제를 팀원마다 다른 방식으로 풀어서 코드베이스가 제각각이 되는 것을 막는 데 있습니다. BE에는 "DB 커밋 전에 알림이 나가면 안 된다", "배치가 하루에 두 번 돌아도 중복 발송되면 안 된다"처럼 여러 도메인에서 반복해서 마주치는 문제가 있고, 그때마다 이미 검증된 해결 방식이 정해져 있습니다. 새 기능을 만들다가 낯익은 문제를 만나면, 새로 고민하기 전에 이 문서에 이미 답이 있는지부터 확인합니다.
