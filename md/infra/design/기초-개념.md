...
- [시스템 아키텍처](시스템-아키텍처.md)에 나오는 UID/GID, 계정 파일, 권한, Kubernetes, NFS, NodePort 기본 개념
...
이 시스템에서는 노드에 마운트된 NFS 홈 경로에 사용한다.

과거에는 GPU 디바이스(`/dev/nvidia*`)도 hostPath로 개별 마운트했으나, 이 방식은 마운트 시점의 디바이스 파일(inode)에 고정되어 호스트에서 드라이버 리로드 등으로 디바이스가 재생성되면 이미 떠 있는 Pod의 GPU 접근이 복구 불가능하게 끊기는 문제가 있었다. 현재는 이 수동 마운트를 제거하고, 이미지에 baked-in된 `NVIDIA_VISIBLE_DEVICES=all`과 노드의 기본 컨테이너 런타임(`nvidia-container-runtime`)이 컨테이너 생성 시점마다 현재 호스트 디바이스 상태를 다시 조회해 자동으로 주입한다. 그래서 Pod 스펙(`volumes`/`volumeMounts`)에는 더 이상 `/dev/nvidia*`가 명시적으로 보이지 않는다 — 컨테이너 안에서 `nvidia-smi`로 실제 GPU 인식 여부를 확인해야 한다.
...
10. config-server API는 내부 API 토큰(`X-Internal-Token`)으로만 변경할 수 있다. 토큰은 실제 인프라를 바꾸는 권한이므로 Secret에만 두고 문서·로그에 남기지 않는다.
