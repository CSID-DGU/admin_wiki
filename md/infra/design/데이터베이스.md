...
| 필요할 때 | 문서 |
| --- | --- |
| 승인부터 Pod 생성까지 전체 흐름을 본다 | [시스템 아키텍처](시스템-아키텍처.md) |
| 운영 중 포트 충돌이나 잔여 기록을 확인한다 | [운영 매뉴얼](../operations/운영-매뉴얼.md) |
| API 응답과 오류 형식을 확인한다 | [API 레퍼런스](../operations/API-레퍼런스.md) |
...
2. 클러스터 모든 namespace의 Service가 쓰는 NodePort도 조회해 사용 중인 목록에 합친다. 이 조회가 실패하면 오류를 로그에 남기고 MySQL 목록만으로 계속 진행한다. 이때 다른 Service가 이미 쓰는 포트를 고르면 Service 생성 단계에서 실패할 수 있다.
3. NodePort 범위인 30000~32767에서 사용하지 않는 포트를 찾는다.
4. 요청한 포트 개수만큼 `nodeport_allocations`에 행을 삽입한다.
5. 모든 삽입이 성공하면 `conn.commit()`으로 트랜잭션을 확정한다.
6. 예외가 발생하면 `conn.rollback()`으로 전체 작업을 취소한다.
...
| `reconcile_nodeport_allocations(namespace)` | MySQL 기록과 Kubernetes Service를 비교해 남은 기록을 정리한다. |
...
접속 방법은 [운영 매뉴얼](../operations/운영-매뉴얼.md)의 관리 주소와 명령 표를 따른다.
