...
FARM NAS 홈 마운트에는 krb5 인증이 전제된다. 자격 증명의 생성·배포·복구 절차는 [kdc-setup](../kdc-setup/index.md)에서 다루며, 이 체인이 끊기면 신규 Pod가 FailedMount로 Ready에 도달하지 못한다(6.3절).
...
FARM NAS 홈 마운트에는 krb5 인증이 전제된다. 자격 증명 체인의 상세 점검·복구는 [kdc-setup 운영 문서](../kdc-setup/operations.md)를 따른다. 이 체인이 끊기면 신규 Pod가 FailedMount로 생성되지 못하므로, 먼저 [운영 매뉴얼](../operations/운영-매뉴얼.md)의 장애 표로 증상을 분류한다.
