...
이 문서는 `admin_infra`의 config-server가 사용자 Pod, 계정, 홈 디렉터리를 만드는 순서를 정리한다. 운영 명령과 장애 조치는 [운영 매뉴얼](../operations/운영-매뉴얼.md), API의 요청·응답은 [API 레퍼런스](../operations/API-레퍼런스.md), Kerberos 설정은 [kdc-setup](../kdc-setup/index.md)에서 확인한다.
...
`PUT /accounts/users`는 `/kube_share`의 `passwd`, `group`, `shadow` 파일에 새 사용자 정보를 쓴다. 새 UID와 GID를 정하는 방법은 [계정 정보](#account-information)에서 확인한다.
