...
| 마이그레이션 | 옮길 노드 선택(옮길 이유가 없으면 건너뜀으로 성공) → 사용자 설정 조회 → 기존 Pod 로그인 비밀번호 이어받기 → Pod 준비 → Pod 스펙 → Pod 생성 → Ready 대기 → 접속 포트 연결 → 기존 Pod 정리 |
...
| `full` | noprobe + 생성 뒤 실제 접근 시험(UID, Kerberos 티켓, 홈 읽기·쓰기, GPU, 외부 SSH)과 회수 뒤 차단 시험. 시험을 통과해야 완료로 기록 |
