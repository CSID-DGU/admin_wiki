...
```bash
curl -fsS -X PUT "$config_server_url/accounts/groups" -H "X-Internal-Token: $token" -H 'Content-Type: application/json' --data '{"name":"그룹명","members":["사용자명"]}'
```

기존 그룹에 사용자를 추가한다.

```bash
curl -fsS -X PUT "$config_server_url/accounts/users/사용자명/groups" -H "X-Internal-Token: $token" -H 'Content-Type: application/json' --data '{"groups":["그룹명"]}'
```
...
신청 기록이 없는 고아 Pod만 `POST /delete-pod`로 지운다(admin_be도 신청이 있는 Pod는 이 경로를 막는다).

```bash
curl -fsS -X POST "$config_server_url/delete-pod" -H "X-Internal-Token: $token" -H 'Content-Type: application/json' --data '{"pod_name":"'"$pod_name"'"}'
```
...
관리자 화면 컨테이너 상세의 마이그레이션이 `POST /migrate`를 부른다. `force`를 켜면 GPU 여유 비교(개선 비율)를 보지 않고 가장 여유 있는 후보 노드로 옮기고, 끄면 개선 비율(기본 0.2)을 넘을 때만 옮긴다.

- 새 Pod가 준비된 뒤에 기존 Pod·Service·포트 배정을 지운다. 새 Pod가 실패하면 기존 Pod는 그대로 둔다.
...
- 새 노드에서 이미지를 처음 받으면 수 분 걸린다(실험 환경 실측: farm2 → farm6 약 4분). 화면 요청 제한(570초)보다 오래 걸리면 화면에는 실패로 보여도 이전은 계속될 수 있으니, 실패로 보이면 Pod 목록을 먼저 확인한다.
...
잔류가 보이면 신청 번호를 찾아 회수 작업을 등록한다(6.2절). 신청 기록이 없는 Pod만 `POST /delete-pod`로 지운다.
