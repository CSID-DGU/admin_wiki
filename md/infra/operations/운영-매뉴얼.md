...
config-server 배포, 계정·Pod 작업, 일상 점검, 문제 확인과 복구 절차를 정리한 운영 문서다. 구조와 용어가 낯설면 [시스템 아키텍처](../design/시스템-아키텍처.md)와 [기초 개념](../design/기초-개념.md)을 먼저 읽는다. Helm 값과 Secret 이름은 [Helm 차트 레퍼런스](Helm-차트-레퍼런스.md)에서 확인하되, **keytab·개인키·비밀번호의 실제 값은 어떤 문서나 로그에도 적지 않는다.**
...
namespace는 Kubernetes 리소스를 나누어 두는 공간이다.
...
주의: 레거시 namespace `containerssh`, `cssh`가 클러스터에 남아 있을 수 있다. 현재 서비스는 사용하지 않으므로, 여기의 리소스를 운영 서비스로 보지 않는다. 삭제는 실제 사용 여부를 확인한 뒤 결정한다.
...
`/health`는 HTTP 프로세스만 확인한다. admin_be, MySQL, Redis, Prometheus, NAS, FARM SSH, AD는 확인하지 않으므로 `/health`가 OK여도 승인이 실패할 수 있다. 관련 서비스를 함께 확인한다.
...
Prometheus는 대시보드용일 뿐 아니라 **Pod를 올릴 노드를 고를 때도 사용한다.** 모니터링이 멈추면 대시보드만 보이지 않는 것이 아니라 노드 선택 결과도 달라질 수 있고, 모든 조회가 실패하면 `create-pod`가 실패한다([시스템 아키텍처](../design/시스템-아키텍처.md)의 GPU 노드 선택 방식).
...
주의: 롤백은 **config-server Deployment만** 이전 버전으로 되돌린다. 계정 파일, MySQL 포트 배정 기록, Kubernetes Service, FARM 노드의 keytab은 자동으로 이전 상태가 되지 않는다. 배포 사이에 실패한 요청이 있으면 해당 요청의 `stage`, `rollback`과 실제 리소스를 함께 확인한다.
...
주의: NAS 홈 삭제가 실패해도 계정 삭제는 계속 진행된다. `deleted` 응답 뒤에도 홈 디렉터리가 남을 수 있으므로 로그와 NAS 경로를 확인한다.
...
주의: Pod가 이미 없다는 성공 응답은 노드 이름을 읽지 못해 Kerberos 노드 정리를 건너뛰었을 수 있다. 이 경우 사용자와 마지막 노드를 기준으로 [kdc-setup 운영 문서](../kdc-setup/operations.md)의 keytab, ccache, timer를 확인한다.
...
주의: 포트 배정 범위(30000~32767)와 외부에서 열어 둔 범위는 다르다. 교내망에서는 되는데 외부에서만 안 되면, [시스템 아키텍처](../design/시스템-아키텍처.md)의 외부 접근 경로에 나온 방화벽 범위를 먼저 확인한다.
