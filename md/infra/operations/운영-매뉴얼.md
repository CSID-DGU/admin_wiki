...
7. **config-server 제어 API는 승인된 경로에서만 접근한다.** 이 API에는 인증이 없으므로([시스템 아키텍처](../design/시스템-아키텍처.md) 1절 운영 경고), WAS와 승인된 운영망 밖으로 노출되지 않게 유지한다.
...
현재 정상 명세는 다음과 같다([시스템 아키텍처](../design/시스템-아키텍처.md) 7.5절의 그림 참조).
...
Prometheus는 관측뿐 아니라 **Pod 배치 노드 선택의 입력**이기도 한다. 모니터링이 죽으면 대시보드가 안 보이는 것에 그치지 않고 Pod 생성의 노드 선택 품질이 저하되며, 전부 죽으면 create-pod 자체가 실패한다([시스템 아키텍처](../design/시스템-아키텍처.md) 2.3절 폴백).
...
계정 작업의 기준 장부는 `/kube_share`의 passwd 파일이다([시스템 아키텍처](../design/시스템-아키텍처.md) 6.1절). 아래 API가 그 장부를 안전하게 고치는 유일한 경로이므로, 파일을 손으로 편집하지 않는다.
...
**수동 삭제 시 kubectl로 Pod만 지우지 않는다.** Service와 MySQL 배정 행이 남고, 배정 정리(reconcile)는 Service 존재를 기준으로 판단하므로 이 유령 배정을 영영 정리하지 못한다([시스템 아키텍처](../design/시스템-아키텍처.md) 4절).
...
⚠️ 포트 배정 범위(30000~32767)와 외부 개방 범위는 다르다. 교내망에서는 되는데 외부에서만 안 되면 개방 범위 문제이다 — [시스템 아키텍처](../design/시스템-아키텍처.md) 5절(방화벽 함정)을 참고한다.
...
- Prometheus가 각 후보의 GPU 지표를 반환하는지 확인한다. exporter가 죽은 노드는 0점으로 오인 선택될 수 있다([시스템 아키텍처](../design/시스템-아키텍처.md) 2.3절).
