...
`kerberos-nfs`는 FARM/LAB에서 AD Kerberos로 사용자를 인증하고, 같은 UID/GID로
NFS 공유 스토리지에 접근하게 하는 구조와 운영 기준을 설명한다. 처음 읽는 사람은
**설계**에서 전체 인증 흐름을 먼저 이해한 뒤 **운영**에서 생성·점검·복구 명령을
확인한다. 과거 장애의 증거와 재현 실험은 **디버깅 로그**에서 확인한다. 환경별
실제 값과 최종 실행 절차는 인프라 저장소의 FARM/LAB canonical runbook에서
확인한다.
