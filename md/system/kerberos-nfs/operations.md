...
이 문서는 현재 구현된 Docker/FARM·LAB 흐름의 일상 점검과 장애 대응 절차를
...
않으면 oneshot service를 즉시 실행하는 것이 현재 구현이다. timer만 존재하고
...
desired fstab 변경은 [server-state playbook](https://github.com/CSID-DGU/admin_infra_server/blob/main/server-state/ansible_playbook/kerberos_nfs_client_recovery.yml)이
소유한다. 이미 mount된 legacy superblock은 유지보수 창에 clean unmount/remount한다.
...
설정 파일의 소유 주체, service 이름과 keytab lifecycle이 다르다. client에서
...
| NFS SPN 소유자 | 전용 user `svc-nfs-farm`; `NAS$`와 분리 | computer account `LAB-STORAGE$` |
...
container가 해당 export를 사용하도록 변경하면 안 된다.
...
- 두 환경 모두 identity/export 변경 후 `exportfs -ra`와 필요한 NFS/RPC cache
  flush를 수행할 수 있지만, 활성 client와 D-state를 확인한 유지보수 작업으로
  취급한다.
