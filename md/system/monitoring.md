...
| 경로 | 핵심 기능 |
| --- | --- |
| `prometheus/config/` | FARM/LAB별 kube-prometheus-stack values와 PV, 공통 storage class |
| `prometheus/gpu/` | 두 cluster가 공유하는 NFD와 NVIDIA device plugin 설정 |
| `prometheus/exporters/cluster-monitor-exporter/` | mount, GPU, Docker, container, 연결성, NFS GSS 지표 |
| `prometheus/exporters/gpu-user-exporter/` | GPU process를 UID DB의 실제 사용자/container에 귀속 |
...
| `health-checks/kerberos-nfs-keytab/` | FARM AD/NAS keytab의 읽기 전용 5분 주기 상태 점검 |
...
- LAB Prometheus는 NodePort `30073`으로 노출하며 FARM Grafana의
  `prometheus-lab` datasource가 이를 조회한다.

cluster별 values 파일을 나눈 이유는 target, retention, storage, Alertmanager
routing과 control plane 장애가 독립적이기 때문이다. 디렉터리는
`prometheus/config/`로 합쳤지만 FARM과 LAB Prometheus 배포는 계속 분리한다.
dashboard는 공통 UI이므로 한 곳에 두되 datasource UID와 label로 FARM/LAB를
분리한다.
...
- `health-checks/kerberos-nfs-keytab/`은 AD/NAS keytab을 읽기 전용으로
  확인한다. keytab repair, service restart와 key rotation은
  `kerberos-nfs/script/`의 명시적 작업으로 유지한다.
