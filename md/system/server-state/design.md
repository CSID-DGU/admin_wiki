...
이 절은 `server-state`를 실제로 쓸 때 필요한 공통 개념을 다룬다. 10개
구성요소 각각의 자세한 내용은 4절에 있다.

| 절 | 다루는 내용 |
| --- | --- |
| 3.1 실행 예시 | 명령 한 줄이 실제로 어떻게 생겼는지 |
| 3.2 구성요소 설명 | 정상 상태·점검·설정·설정 실행(실행 형태·안전 수준)이 무슨 뜻인지 |
| 3.3 `describe` | 서버에 접속하지 않고 구성요소 정의 내용만 보기 |
| 3.4 `audit` | 서버를 점검만 하기(읽기 전용) |
| 3.5 `plan` | 실제로 바꾸기 전에 무엇이 바뀔지 미리 보기 |
| 3.6 `apply` | 실제로 서버 상태를 바꾸기 |
...
| 항목 | 의미 |
| --- | --- |
| <a id="def-desired-state"></a>정상 상태 | 이 구성요소가 "문제 없음"으로 판정하는 서버 상태. `components/<id>.yml`의 `desired_state` 값 그대로이며, `describe` 명령으로 그 원문을 바로 볼 수 있다(파일 예시는 5.1절) |
