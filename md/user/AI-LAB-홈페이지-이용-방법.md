...
- **Ubuntu 사용자명** : 서버에 로그인할 때 쓸 계정 이름이다. 3~32자로, 소문자로 시작하고 소문자·숫자·하이픈(-)만 쓸 수 있으며 소문자나 숫자로 끝나야 한다. `root`·`docker`·`sudo`·`www-data`처럼 서버가 이미 쓰는 시스템 계정·그룹 이름과, 이미 만들어진 공유 그룹과 같은 이름은 쓸 수 없다. "다음"을 누르면 쓸 수 있는 이름인지 자동으로 확인해 준다.
